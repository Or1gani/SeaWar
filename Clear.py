...
class Game():
    ...
    def create_move(player_move, field, cs, direct):
        ...
        if cs == "1":
            print(f"\033[1m\033[31m{'Корабль: Большой @@@'}\033[0m")
            ship_size = 3
        elif cs == "2":
            print(f"\033[1m\033[31m{'Корабль: Средний @@'}\033[0m")
            ship_size = 2
        else:
            print(f"\033[1m\033[31m{'Корабль: Малый @'}\033[0m")
            ship_size = 1
        if direct == "1":
            print(f"\033[1m\033[31m{'Направление: Горизонтальный'}\033[0m")
        else:
            print(f"\033[1m\033[31m{'Направление: Вертикальный'}\033[0m")
        cord = Game.get_cords()
        if direct == "1":
        ...
        if flag:
            if 6 - y >= ship_size:
                for k in range(y, ship_size + y):
                    field[x][k] = f"\033[1m\033[31m{'B'}\033[0m"
        ...
        if flag:
            if 6 - x >= ship_size:
                for k in range(x, ship_size + x):
                    field[k][y] = f"\033[1m\033[31m{'B'}\033[0m"
                    ship_coordinates.append(k)
    ...
    def choose_ship(player_name, bs, ms, ss):
        ...
        while bs != 0 or ms != 0 or ss != 0:
            print("\n",f"Большие: {bs}. Средние: {ms}. Малые: {ss}.", "\n")
    ...
    def shoot(mark_field, current_move, cords, field):
        if field[cords[0]][cords[1]] == "#":
            ...
            print("\n",'ПОПАДАНИЕ!',"\n")
            mark_field[cords[0]][cords[1]] = "●"
            if current_move == player1.player_name:
                for key, value in field2.ship_location.items():
                    for i in range(0, len(value) - 1, 2):
                        if cords[0] == value[i] and cords[1] == value[i + 1]:
                            value[i], value[i+1] = "-", "-"
                            field2.ship_location[key] = value
                for key, value in field2.ship_location.items():
                    for i in value:
                        if i == "-":
                            flag = True
                ...
                if flag:
                    ...
                    player2.alive_ships -= 1
                    print("\n", "УБИЛ!", "\n")
                    return 3

            else:
                for key, value in field1.ship_location.items():
                    for i in range(0, len(value) - 1, 2):
                        if cords[0] == value[i] and cords[1] == value[i + 1]:
                            value[i], value[i + 1] = "-", "-"
                            field1.ship_location[key] = value
                for key, value in field1.ship_location.items():
                    for i in value:
                        if i == "-":
                            flag1 = True
...
class Main():
    ...
    print(f"\033[1m\033[31m{'Расстановка кораблей'}!\033[0m", "\n", f"Ходит игрок: {game1.current_move}", "\n")
    ...
    game1.clearConsol()
    print(f"\033[1m\033[31m{'БОЙ НАЧИНАЕТСЯ'}!\033[0m")
    game1.load()
    game1.clearConsol()
    current_field = field2.field
    game1.current_move = player1.player_name
    ...
    while player1.alive_ships != 0 and player2.alive_ships != 0:
        ...
        cords[1] = cords[1] -1
        result = game1.shoot(current_mark_field, current_move, cords, current_field)
        if result == 0:
            flag = True
            count += 1
            print("Переход хода")
            ...
            flag = True
            print(f"Количество живых кораблей у игрока {player1.player_name}: {player1.alive_ships}", "\n")
            print(f"Количество живых кораблей у игрока {player2.player_name}: {player2.alive_ships}", "\n")
    if player1.alive_ships == 0:
        print(f"\033[1m\033[31m{f'Победил Игрок: {player2.player_name}'}\033[0m")
    else:
